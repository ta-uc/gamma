import math
from od_data_init import actual as traffic_before, loss as loss_before
...
def estimate_gamma():
  ...
  for od in traffic_before.keys():
    ...
    od_packet_loss_after = loss_after[od]   # 経路制御前観測パケットロス率
    if (od_packet_loss_before - od_packet_loss_after) != 0 and od_traffic_before / od_traffic_after > 0:
      if math.log((od_traffic_before / od_traffic_after)) / (od_packet_loss_before - od_packet_loss_after) > 0:
        gamma.append(0)
      else:
