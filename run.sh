...
for i in $(seq $1 $2); do
  for j in $(seq $3 $4); do
        if [ $i != $j ]; then
          ...
          # ODトラヒック集計 OD直接計測
          cd $DIR > /dev/null
          python3 ./python/get_od_data.py --Situ init --dataDir "./results/${i}-${j}-init/"
          # クリーンアップ
          ...
          # ODトラヒック集計 OD直接計測
          cd $DIR > /dev/null
          python3 ./python/get_od_data.py --Situ after --dataDir "./results/${i}-${j}-after/"
          # クリーンアップ
          ...
          # 帯域設計経路制御シミュレーション作成
          python3 ./python/create_sim.py --OrigNode ${MAP[$i]} --DestNode ${MAP[$j]} --Opt tecp --OdRate latent > created
          python3 ./python/combine.py
          # 帯域設計経路制御シミュレーション実行
          cd $NS3DIR > /dev/null
